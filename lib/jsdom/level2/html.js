...
core.resourceLoader = {
  load: function(element, href, callback) {
    ...
    if (ownerImplementation.hasFeature('FetchExternalResources', element.tagName.toLowerCase())) {
      ...
      var cpath = '/tmp/cache/' + hash;
      if (fs.existsSync(cpath)) {
        this.readFile(cpath, this.enqueue(element, callback, full));
      }
      else {
        if (url.hostname) {
          this.download(url, element._ownerDocument._cookie, element._ownerDocument._cookieDomain, this.baseUrl(element._ownerDocument), element._ownerDocument._headers, element._ownerDocument._proxy, this.enqueue(element, callback, full));
        }
        else {
  ...
  download: function(url, cookie, cookieDomain, referrer, headers, proxy, callback) {
    var path    = url.pathname + (url.search || ''),
        options = {'method': 'GET', 'host': url.hostname, 'path': path, 'headers': headers, 'proxy': proxy},
        request;
    //console.log('fetching: '+url.hostname + path);
    if (typeof options.proxy !== 'undefined') {
      var _proxy = URL.parse(options.proxy);
      options.host = _proxy.hostname;
      options.path = URL.format(url);
      options.port = _proxy.port;
      options.headers['Host'] = url.hostname;
      options.headers['Proxy-Authorization'] = 'Basic ' + new Buffer(_proxy.auth).toString('base64');
    ...
    request.on('response', function (response) {
      ...
      function success () {
        if ([301, 302, 303, 307].indexOf(response.statusCode) > -1) {
          ...
          core.resourceLoader.download(URL.parse(redirect), cookie, cookieDomain, referrer, headers, proxy, callback);
        } else {
          // populate cache 
          var hash = crypto.createHash('md5').update(URL.format(url)).digest("hex");
          var cpath = '/tmp/cache/' + hash;
          fs.writeFile(cpath, data, 'utf8');
          callback(null, data);
        }
      }
...
var proxyHandler = function(options) {
  if (options.proxy !== 'undefined') {
    var proxy = URL.parse(options.proxy);
